 ...
 /* set up node building blocks for the selection list of days */
 function setupDays() 
 {
 ...
 function updateDays() {
		...
		else if(selectedMonth === "4" || selectedMonth === "6" || selectedMonth === "9" || selectedMonth === "11")
		{
			...
			deliveryDay.appendChild(thirty.cloneNode(true));
				...
				selectedMonth === "7" || selectedMonth === "8" || selectedMonth === "10" || selectedMonth === "12")
				{
 ...
 /* automatically check Custom message check box if user makes entry to customText box */
 ...
 function createEventListeners()
 {
	...
	{
		same.attachEvent("onclick", billingAddress);
	}
	...
	function validateAddress(fieldsetId)
	{
		...
		try
		{
			...
			if(fieldsetValidity === false)
			{
				...
				{
					throw "Please complete all Delivery Address inforamtion";
				}
			}
			else
			{
		...
		try
		{
			if(!cards[0].checked && !cards[1].checked && !cards[2].checked && !cards[3].checked)
				...
				if(ccNumElement.value === "")
				{
					...
					ccNumElement.style.backgroud = "rgb(255,233,233)";
					fieldsetValidity = false;
				}
				else
				{
		...
		try
		{
			...
			if((usernameElement.value !== "" && pass1Element.value !== "" && pass2Element.value !== ""))
			{
				...
				if(pass1Element.value !== pass2Element.value)
				{
					...
					throw "Passwords entered do not match.";
				}
			}
			if(!(usernameElement.value === "" && pass1Element.value === "" && pass2Element.value === ""))
				{
 ...
 /* run setup function when the page finishes loading */
